...
# @File         : codes_单线程_觅知网_T7200室内音频素材爬虫.py
# @Mark         :
                # 1.合入多线程爬虫代码
...
class GetMp3(object):
    def getCatsound(self):
        ...
        logger.add(f'./log/{timeinfo}_codes_单线程_veer_T7200室内音频素材爬虫.log', rotation='50MB', encoding='utf-8',
        ...
        for num in range(1, 4): # 最大页码数
            urlList = f'https://www.51miz.com/so-sound/86834/p_{num}/'
            soundLinkAfterList1 = []
            ...
            for j in range(30):
                ...
                for root, dirs, files in os.walk(f'./result'):
                    ...
                    if filesCount < 84:  # 增加判断，给音频下载增加时间，不然获取了84个音频下载的连接，但下载的音频数远远小于84
                        time.sleep(10)
